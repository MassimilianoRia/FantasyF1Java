...
public final class AdminService {
    ...
    public void removeEdition(final int editionId) {
        transactions.inTransaction(connection -> {
            requireLockedEdition(connection, editionId);
            if (
                admin.countWeekends(connection, editionId) > 0
                    || admin.countConstructors(connection, editionId) > 0
                    || admin.countDrivers(connection, editionId) > 0
            ) {
                throw ServiceGuards.conflict(
                    "Prima di eliminare l'edizione rimuovi tutti i weekend, "
                        + "i piloti iscritti e le scuderie iscritte."
                );
            }
            if (
                admin.countEditionTeams(connection, editionId) > 0
                    || admin.countEditionLeagues(connection, editionId) > 0
            ) {
                throw ServiceGuards.conflict(
                    "L'edizione è già usata da team fantasy o leghe e non "
                        + "può essere eliminata."
                );
            }
            admin.deleteEdition(connection, editionId);
        });
    }

    public void removeGrandPrix(final int grandPrixId) {
        if (grandPrixId <= 0) {
            throw ServiceGuards.invalid("Seleziona un Gran Premio.");
        }
        transactions.inTransaction(connection -> {
            if (!admin.isGrandPrixPresent(connection, grandPrixId)) {
                throw ServiceGuards.notFound("Gran Premio non trovato.");
            }
            if (admin.countGrandPrixWeekends(connection, grandPrixId) > 0) {
                throw ServiceGuards.conflict(
                    "Il Gran Premio è inserito nel calendario di almeno "
                        + "un'edizione. Rimuovi prima tutti i relativi weekend."
                );
            }
            admin.deleteGrandPrix(connection, grandPrixId);
        });
    }
    ...
    public void removeWeekend(
        ...
        processing.removeWeekend(editionId, grandPrixId);
    }

    public void removeConstructor(final int constructorId) {
        if (constructorId <= 0) {
            throw ServiceGuards.invalid("Seleziona una scuderia.");
        }
        transactions.inTransaction(connection -> {
            if (!admin.isConstructorPresent(connection, constructorId)) {
                throw ServiceGuards.notFound("Scuderia non trovata.");
            ...
            if (
                admin.countConstructorEnrollments(
                    connection,
                    constructorId
                ) > 0
            ) {
                throw ServiceGuards.conflict(
                    "La scuderia è iscritta ad almeno un'edizione. "
                        + "Rimuovi prima tutte le sue iscrizioni."
                );
            }
            admin.deleteConstructor(connection, constructorId);
        });
    }

    public void removeConstructorEnrollment(
        final int editionId,
        final int constructorId
    ) {
        transactions.inTransaction(connection -> {
            ...
            if (!admin.isConstructorEnrolled(
                ...
                throw ServiceGuards.notFound(
                    "Iscrizione della scuderia non trovata."
                );
            }
            if (
                admin.countDriversForConstructor(
                ...
                throw ServiceGuards.conflict(
                    "La scuderia ha ancora piloti iscritti. Rimuovi prima "
                        + "le iscrizioni dei suoi piloti."
                );
            }
            admin.deleteConstructorEnrollment(
                connection,
                editionId,
                constructorId
            );
    ...
    public void removeDriver(final int driverId) {
        if (driverId <= 0) {
            throw ServiceGuards.invalid("Seleziona un pilota.");
        }
        transactions.inTransaction(connection -> {
            if (!admin.isDriverPresent(connection, driverId)) {
                throw ServiceGuards.notFound("Pilota non trovato.");
            }
            if (admin.countDriverEnrollments(connection, driverId) > 0) {
                throw ServiceGuards.conflict(
                    "Il pilota è iscritto ad almeno un'edizione. "
                        + "Rimuovi prima tutte le sue iscrizioni."
                );
            }
            admin.deleteDriver(connection, driverId);
        });
    }

    public void removeDriverEnrollment(
        final int editionId,
        final int driverId
    ) {
        transactions.inTransaction(connection -> {
            requireLockedEdition(connection, editionId);
            if (!admin.isDriverEnrolled(
                connection,
                editionId,
                driverId
            )) {
                throw ServiceGuards.notFound(
                    "Iscrizione del pilota non trovata."
                );
            }
            if (
                admin.countDriverPerformances(
                    connection,
                    editionId,
                    driverId
                ) > 0
                ...
                throw ServiceGuards.conflict(
                    "Il pilota ha ancora prestazioni sportive registrate. "
                        + "Rimuovile prima di eliminare l'iscrizione."
                );
            }
            if (
                admin.countDriverTeamSelections(
                    connection,
                    editionId,
                    driverId
                ) > 0
                ...
                throw ServiceGuards.conflict(
                    "Il pilota è già stato selezionato in un team fantasy "
                        + "e la sua iscrizione non può essere eliminata."
                );
            }
            admin.deleteDriverEnrollment(
                connection,
                editionId,
                driverId
            );
